...
// produce a perl module name from the file name
static QString pm(const QString &name)
{
    QStringList parts = name.split(QLatin1Char(':'));
    QString retval = parts.first();
    ...
    if (!retval.endsWith(QLatin1String(".pm")))
        retval.append(QLatin1String(".pm"));
...
static void writeArgTypesList(QTextStream &ts, const QStringList &/*argNames*/,
...
static void writeAdaptor(const QString &filename, const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    foreach (const QDBusIntrospection::Interface *interface, interfaces) {
        ...
        cs << "sub NEW" << endl
           << "{" << endl
           << "    # constructor" << endl
           << "    my ( $class, $parent, $data ) = @_;" << endl
           << "    $class->SUPER::NEW($parent);" << endl
           << "    if ( defined $data ) {" << endl
           << "        this->{data} = $data;" << endl
           << "    } else {" << endl
           << "        this->{data} = $parent;" << endl
           << "    }" << endl
           << "    this->setAutoRelaySignals(1);" << endl
           << "}" << endl
           << endl;
